...
double FDMEulerExplicit::calcPrice(double spotPrice) {

	double diff_low = 1.0;
	double diff_high = 1.0;
	double v_low, v_high, s_low, s_high;
	...
	while (cur_t < t_dom) {
		...
		if (cur_t >= t_dom )
		{
			//find highest price lower than spot
			//find lowest price higher than spot
			

			for (int j = 0; j < J; j++) {
				if (fabs(spotPrice - x_values[j]) < diff_low & spotPrice > x_values[j])
				{
					diff_low = spotPrice - x_values[j];
					v_low = new_result[j];
					s_low = x_values[j];

				}
				
				if (fabs(x_values[j] - spotPrice) < diff_high & spotPrice <= x_values[j])
				{
					diff_high = x_values[j] - spotPrice;
					v_high = new_result[j];
					s_high = x_values[j];
				}
			}
	...
	double price = ((s_high - spotPrice)* v_low + (spotPrice - s_low)*v_high) / (s_high - s_low);
